	...
	items: [
		{
					...
					features: [
						{
							...
							collapsible: false,
							groupHeaderTpl:
								'{columnName:uppercase}: {name:capitalize} ({rows.length} {[values.rows.length > 1 ? "' +
								locale["app.main[9]"] +
								'" : "' +
								locale["app.main[8]"] +
								'"]})'
					...
					columns: [
						{
							...
							tpl:
								'<img src="{[ values.type !== "custom" ? "resources/icons/"+values.logo : (values.logo == "" ? "resources/icons/custom.png" : values.logo) ]}" data-qtip="{type:capitalize}" width="32" style="{[ values.enabled ? "-webkit-filter: grayscale(0)" : "-webkit-filter: grayscale(1)" ]}" />'
						},
						{
							dataIndex: "name",
							variableRowHeight: true,
			...
			tbar: {
				...
				items: [
					{
						glyph: JSON.parse(localStorage.getItem("dontDisturb"))
							? "xf1f7@FontAwesome"
							: "xf0f3@FontAwesome",
						text:
							locale["app.main[16]"] +
			...
			bbar: [
				...
				"->",
				{
					xtype: "label",
					html:
						'<span class="fa fa-code" style="color:black;"></span> ' +
						locale["app.main[26]"] +
						' <span class="fa fa-heart" style="color:red;"></span> ' +
						locale["app.main[27]"].replace(
							"Argentina",
							'<img src="resources/flag.png" alt="Argentina" data-qtip="Argentina" />'
						)
				},
				"->"
			]
